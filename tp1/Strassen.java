import java.io.FileNotFoundException;
import java.io.IOException;

public class Strassen {

	private double timeExecution = 0;
	private int dimension = 0;
	private int seuil = 0;
	private int matrix[][] = new int[0][0];

	public Strassen(Matrix mA, Matrix mB, int seuil) throws IOException {

		if (validateDimension(mA.getDim(), mB.getDim())) {
			this.seuil = seuil;
			this.matrix = new int[this.dimension][this.dimension];
			double startTime = System.nanoTime();

			multiply(mA.matrix, mB.matrix);

			showTimeExecuted(startTime);

		}
	}

	public void showTimeExecuted(double startTime){
		double endTime = System.nanoTime();
		double elapsedTime = endTime - startTime;

		// this.timeExecution = elapsedTime / 1000000000; // en secondes
		// this.timeExecution = elapsedTime / 1000000; // en millisecondes
		this.timeExecution = elapsedTime / 1000; // en microseconds

		System.out.println("Strassen - Le temps d'execution en nanosecondes: " + elapsedTime);
		System.out.println("Strassen - Le temps d'exectuion en secondes: " + this.timeExecution);
		System.out.println("Strassen - Taille de la matrice: " + dimension + "x" + dimension);
		System.out.println("-----------------------------------");
	}

	public double getTimeExecution() {
		return this.timeExecution;
	}

	public int getDim() {
		return this.dimension;
	}

	public boolean validateDimension(int dimA, int dimB) throws FileNotFoundException {

		if (dimA == dimB) {
			this.dimension = dimA;
			return true;
		} else {
			System.out.println("dimension <" + dimA + ":" + dimB + "> incorrectee!!!!!!!!");
			return false;
		}

	}

	public int[][] multiply(int[][] A, int[][] B) throws IOException {

		int n = A.length;
		int[][] R = new int[n][n];

		/** base case **/
		if (n <= this.seuil) {
			Conventional conv = new Conventional(R);
			R = conv.multiplyMatrices(A, B);

			// R[0][0] = A[0][0] * B[0][0];
			// for (int i = 0; i < n; i++) {
			// 	for (int j = 0; j < n; j++) {
			// 		R[i][j] = 0;
			// 		for (int k = 0; k < n; k++) {
			// 			R[i][j] += A[i][k] * B[k][j];
			// 		}
			// 	}
			// }

		} else {
			int[][] A11 = new int[n / 2][n / 2];
			int[][] A12 = new int[n / 2][n / 2];
			int[][] A21 = new int[n / 2][n / 2];
			int[][] A22 = new int[n / 2][n / 2];
			int[][] B11 = new int[n / 2][n / 2];
			int[][] B12 = new int[n / 2][n / 2];
			int[][] B21 = new int[n / 2][n / 2];
			int[][] B22 = new int[n / 2][n / 2];

			/** Divisier la matrice A en 4 moitiers **/
			split(A, A11, 0, 0);
			split(A, A12, 0, n / 2);
			split(A, A21, n / 2, 0);
			split(A, A22, n / 2, n / 2);
			/** Divisier la matrice B en 4 moitiers **/
			split(B, B11, 0, 0);
			split(B, B12, 0, n / 2);
			split(B, B21, n / 2, 0);
			split(B, B22, n / 2, n / 2);

			/**
			 * M1 = (A11 + A22)(B11 + B22) M2 = (A21 + A22) B11 M3 = A11 (B12 - B22) M4 =
			 * A22 (B21 - B11) M5 = (A11 + A12) B22 M6 = (A21 - A11) (B11 + B12) M7 = (A12 -
			 * A22) (B21 + B22)
			 **/

			int[][] M1 = multiply(add(A11, A22), add(B11, B22));
			int[][] M2 = multiply(add(A21, A22), B11);
			int[][] M3 = multiply(A11, sub(B12, B22));
			int[][] M4 = multiply(A22, sub(B21, B11));
			int[][] M5 = multiply(add(A11, A12), B22);
			int[][] M6 = multiply(sub(A21, A11), add(B11, B12));
			int[][] M7 = multiply(sub(A12, A22), add(B21, B22));

			/**
			 * C11 = M1 + M4 - M5 + M7 C12 = M3 + M5 C21 = M2 + M4 C22 = M1 - M2 + M3 + M6
			 **/
			int[][] C11 = add(sub(add(M1, M4), M5), M7);
			int[][] C12 = add(M3, M5);
			int[][] C21 = add(M2, M4);
			int[][] C22 = add(sub(add(M1, M3), M2), M6);

			/** joindre 4 moitiés en une seule matrice de résultats **/
			join(C11, R, 0, 0);
			join(C12, R, 0, n / 2);
			join(C21, R, n / 2, 0);
			join(C22, R, n / 2, n / 2);
		}

		/** return result **/
		this.matrix = R;
		return R;
	}

	/** Fonction pour soustraire deux matrices **/
	public int[][] sub(int[][] A, int[][] B) {
		int n = A.length;
		int[][] C = new int[n][n];
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				C[i][j] = A[i][j] - B[i][j];
		return C;
	}

	/** Fonction pour ajouter deux matrices **/
	public int[][] add(int[][] A, int[][] B) {
		int n = A.length;
		int[][] C = new int[n][n];
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				C[i][j] = A[i][j] + B[i][j];
		return C;
	}

	/** Fonction pour diviser la matrice parent en matrices enfants **/
	public void split(int[][] P, int[][] C, int iB, int jB) {
		for (int i1 = 0, i2 = iB; i1 < C.length; i1++, i2++)
			for (int j1 = 0, j2 = jB; j1 < C.length; j1++, j2++)
				C[i1][j1] = P[i2][j2];
	}

	/** Fonction pour joindre les matrices enfants à la matrice parent **/
	public void join(int[][] C, int[][] P, int iB, int jB) {
		for (int i1 = 0, i2 = iB; i1 < C.length; i1++, i2++)
			for (int j1 = 0, j2 = jB; j1 < C.length; j1++, j2++)
				P[i2][j2] = C[i1][j1];
	}

	public void displayMatrix() {
		for (int i = 0; i < this.dimension; i++) {
			for (int j = 0; j < this.dimension; j++) {
				System.out.print(this.matrix[i][j] + "\t");
			}
			System.out.println();
		}

		System.out.println("--------------------------------");
	}

}
